...
$sql_anggota = "
    SELECT v.*, a.kode_pos, ap.id_jabatan 
...
<style>
    ...
    .status-aktif {
        color: #00FF66; border: 1px solid #00FF66;
    ...
    .status-alumni {
        color: #facc15; border: 1px solid #facc15;
    ...
    <div class="pt-6 px-4" style="display: flex; flex-direction: column; gap: 35px;">
        ...
        <?php if (isset($_GET['msg'])): ?>
            <?php 
                ...
                $text = '';
                if ($msg == 'success') $text = 'Data anggota berhasil ditambahkan!';
                elseif ($msg == 'updated') $text = 'Data anggota berhasil diperbarui!';
                elseif ($msg == 'deleted') $text = 'Data anggota berhasil dihapus!';
        ...
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <div>
                ...
                <p style="color: var(--text-muted); font-size: 12px; margin-top: 5px;">Total Anggota Terdaftar: <?= count($resAnggota) ?></p>
            </div>
            <?php if ($isAdmin): ?>
                <button onclick="toggleModal('addAnggotaModal')" style="background: #00d2ff; color: #000; font-weight: 800; padding: 12px 25px; border-radius: 12px; border:none; cursor:pointer;">+ Anggota Baru</button>
            <?php endif; ?>
        </div>

        <!-- FILTER FORM -->
        <form method="GET" action="anggota.php" class="glass-card" style="padding: 20px; display: flex; gap: 15px; align-items: center; flex-wrap: wrap;">
            ...
            <div style="display: flex; flex-direction: column; gap: 5px; min-width: 150px; flex: 1;">
                ...
                <select name="divisi" class="form-input" style="padding: 10px;">
                    ...
                    <option value="Lintas Divisi" <?= ($filterDivisi == 'Lintas Divisi') ? 'selected' : '' ?>>Lintas Divisi</option>
            ...
            <div style="display: flex; flex-direction: column; gap: 5px; justify-content: flex-end; height: 100%;">
                ...
                <div style="display: flex; gap: 10px;">
                    <button type="submit" style="background: #facc15; color: #000; padding: 10px 20px; border-radius: 8px; border:none; font-weight: 800; cursor:pointer;">Filter</button>
                    <a href="anggota.php" style="background: rgba(255,255,255,0.1); color: #fff; padding: 10px 20px; border-radius: 8px; text-decoration:none; font-weight: 600;">Reset</a>
                </div>
            </div>
        </form>
            ...
            <table style="width: 100%; border-collapse: collapse; color: #fff;">
                <thead>
                    <tr style="text-align: left; background: rgba(255,255,255,0.02); border-bottom: 1px solid var(--glass-border);">
                        ...
                        <th style="padding: 20px; color: var(--text-muted); font-size: 11px; text-align: center;">STATUS</th>
                        <th style="padding: 20px; color: var(--text-muted); font-size: 11px; text-align: right;">AKSI</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr style="border-bottom: 1px solid rgba(255,255,255,0.03);">
                        <td style="padding: 20px;">
                            ...
                            <div style="font-size: 11px; color: var(--text-muted);"><?= htmlspecialchars($a['nim']) ?> | <?= htmlspecialchars($a['angkatan']) ?></div>
                        </td>
                        <td style="padding: 20px;">
                            <div style="color: #00d2ff; font-weight: 600; font-size: 13px;"><?= htmlspecialchars($a['nama_jabatan'] ?? 'Anggota') ?></div>
                        </td>
                        <td style="padding: 20px; text-align: center;">
                            <?php $stClass = ($a['status_keanggotaan'] == 'Aktif') ? 'status-aktif' : 'status-alumni'; ?>
                            <span class="badge-neon <?= $stClass ?>"><?= htmlspecialchars($a['status_keanggotaan']) ?></span>
                        </td>
                        <td style="padding: 20px; text-align: right;">
                            <?php if ($isAdmin): ?>
                                <div style="display: flex; gap: 15px; justify-content: flex-end;">
                                    <button onclick='openEditModal(<?= json_encode($a, JSON_HEX_APOS | JSON_HEX_QUOT) ?>)' style="background:none; border:none; color:#facc15; cursor:pointer;">
                                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                                    </button>
                                    <a href="admin/anggota_action.php?action=hapus&id=<?= $a['id_anggota'] ?>" onclick="return confirm('Hapus anggota ini?')" style="color:#FF3131;">
    ...
    <div class="glass-card" style="max-width: 700px; width: 100%; max-height: 90vh; overflow-y: auto; padding: 30px;">
        ...
        <form action="admin/anggota_action.php" method="POST">
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
                ...
                <input type="password" name="password" placeholder="Password Akun" class="form-input" required>
                <input type="text" name="angkatan" placeholder="Angkatan (Contoh: 2025)" class="form-input">
                <select name="id_jabatan" class="form-input" required>
                ...
                <div style="grid-column: span 2;">
                    <select name="kode_pos" class="form-input" required>
                        <?php foreach($kodePosList as $kp): ?>
                            <option value="<?= $kp['kode_pos'] ?>"><?= $kp['kode_pos'] ?> - <?= $kp['kecamatan'] ?> (<?= $kp['kelurahan'] ?>)</option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            <div style="display:flex; gap:15px; margin-top:30px;">
                <button type="submit" name="tambah_anggota" style="flex:1; background:#00d2ff; color:#000; padding:15px; border-radius:12px; border:none; font-weight:800; cursor:pointer;">SIMPAN DATA</button>
                <button type="button" onclick="toggleModal('addAnggotaModal')" style="flex:1; background:rgba(255,255,255,0.05); color:#fff; padding:15px; border-radius:12px; border:none; cursor:pointer;">BATAL</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="glass-card" style="max-width: 700px; width: 100%; max-height: 90vh; overflow-y: auto; padding: 30px;">
        ...
        <form action="admin/anggota_action.php" method="POST">
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
                ...
                <select name="id_jabatan" id="edit_id_jabatan" class="form-input" required>
                    <?php foreach($jabatanList as $j): ?>
                        <option value="<?= $j['id_jabatan'] ?>"><?= $j['nama_jabatan'] ?></option>
                    <?php endforeach; ?>
                </select>
            ...
            <div style="display:flex; gap:15px; margin-top:30px;">
                <button type="submit" name="edit_anggota" style="flex:1; background:#facc15; color:#000; padding:15px; border-radius:12px; border:none; font-weight:800; cursor:pointer;">UPDATE DATA</button>
                <button type="button" onclick="toggleModal('editAnggotaModal')" style="flex:1; background:rgba(255,255,255,0.05); color:#fff; padding:15px; border-radius:12px; border:none; cursor:pointer;">BATAL</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    function openEditModal(data) {
        ...
        document.getElementById('edit_id_jabatan').value = data.id_jabatan || '';
